defmodule Plasm.Database.MySql do
    ...
    def create_connection_string(server, port \\ 3306, user_name, password,version, database_name) do
        "Driver={MySQL ODBC #{version} UNICODE Driver};Server=#{server};Port=#{port};Database=#{database_name};User=#{user_name};Password=#{password};Option=3;"
    end
    ...
    def tables(conn, db_name) do
        case get_tables(conn, "show tables from #{db_name}") do
            {:ok, tables} -> {:ok, tables |> Enum.map(fn(x) -> elem(x,0) |> convert_to_utf8 end) }
            {_, msg} -> {:error, msg}
        end
    end
